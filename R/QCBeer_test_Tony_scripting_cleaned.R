...
xcmsSetFragments <- function(xs, cdf.corrected = FALSE, min.rel.int = 0.01, max.frags = 8, msnSelect=c("precursor_int", "ms1_rt", "all"),
				specFilter=c("none", "specPeaks", "cor"), match.ppm = 300, sn = 3, mzgap = 0.2, min.r = 0.75, min.diff = 10) 
{
...
for(NumXcmsPath in 1:paths)
	...
	if(cdf.corrected)
		...
		precursor.prec <- max(sapply(xr@msnPrecursorMz,function(x){nchar(strsplit(as.character(x), "\\.")[[1]][2])}))
	...
	for(i in 1:nrow(ms1peaks))
		...
		if(ms1peaks[i,"sample"]==NumXcmsPath)
			...
			msn.idx <- which(precursor.mz >= npMinMz[i] & precursor.mz <= npMaxMz[i] & msn.rt >= npMinRt[i] & msn.rt <= npMaxRt[i])
			...
			if(length(msn.idx) > 0)
				...
				if(msnSelect=="precursor_int")
					...
					MzTable <-  new("matrix", ncol=2,nrow=length(xr@env$msnMz[start.id:end.id]), data=c(xr@env$msnMz[start.id:end.id], xr@env$msnIntensity[start.id:end.id]))
				...
				if(msnSelect=="ms1_rt")
					...
					MzTable <-  new("matrix", ncol=2,nrow=length(xr@env$msnMz[start.id:end.id]), data=c(xr@env$msnMz[start.id:end.id], xr@env$msnIntensity[start.id:end.id]))
					colnames(MzTable) <- c("mz","intensity")
					MzTable <- MzTable[which(MzTable[,"intensity"]/max(MzTable[,"intensity"]) > min.rel.int),,drop=F]
					}
				...
				if(msnSelect=="all")
					...
					for(msn.id in msn.idx)
						...
						c.MzTable <- rbind(c.MzTable,cbind(msn.id=rep(msn.id,length(xr@env$msnMz[start.id:end.id])), mz=xr@env$msnMz[start.id:end.id], intensity=xr@env$msnIntensity[start.id:end.id]))
						}

					#create an intensity-weighted mz vector and an intensity matrix filled within match.ppm, with missing mz vals as NA 
					weighted.mz <- numeric()
					msn.intensity <- list()
					...
					while(any(!is.na(int)))
						...
						mzmin <- c.MzTable[max.idx,"mz"]-(match.ppm/1000000*c.MzTable[max.idx,"mz"])
						mzmax <- c.MzTable[max.idx,"mz"]+(match.ppm/1000000*c.MzTable[max.idx,"mz"])
						...
						msn.intensity[[count]] <- c.MzTable[mz.idx,,drop=F][match(msn.idx,c.MzTable[mz.idx,"msn.id",drop=F]),"intensity"]
						int[mz.idx] <- NA
						}
				...
				if(specFilter=="cor")
					...
					if(nrow(msn.intensity) > 4)
						...
						cor.summary <- cbind(mz=weighted.mz,intensity=mean.msn.intensity,r=cor.result$r[2:nrow(cor.result$r),1], n=cor.result$n[2:nrow(cor.result$n),1], p=cor.result$P[2:nrow(cor.result$P),1])
						npeaks <- cor.summary[which(cor.summary[,"r"] > min.r & cor.summary[,"n"] >= 4 & cor.summary[,"p"] < 0.05),c("mz","intensity"),drop=F]
						} else
						{
...
fragmentColnames <- c("peakID", "MSnParentPeakID","msLevel","rt", "mz", "intensity", "Sample","GroupPeakMSn", "CollisionEnergy")
...
object@peaks  <- new("matrix", nrow = length(npMz), ncol = length(fragmentColnames),
                         data=c(npPeakID,npMSnParentPeakID,npMsLevel,npRt,npMz,npIntensity,npSample,npGroupPeakMSn, npCollisionEnergy))
...
##keep ms1 peaks with intensity > 300000
ms1 <- ms1[which(ms1$intensity > 300000),]
...
write.table(hm, file="heatmap_QCBeer_methodMS1.txt", col.names=T, row.names=T, sep="\t")
